...
把已验证的跨项目错误经验做成可检索、可维护的通用 Agent Skill。它只在具体工具或环境故障、修复失败、用户指出重复犯错或明确要求时检索；当前上下文已有直接相关经验时也可复用。普通开发和首次修复业务 BUG 不默认检索。只沉淀根因已确认、修复已验证且非显然的跨项目经验。
...
入口采用渐进加载：满足触发条件才读取 `SKILL.md`，同一任务、同一问题默认只检索一次；仅在出现新错误、新根因线索或写入去重需要时补充检索。无相关命中就继续排查，不默认读全文。同一上下文中的已读内容和检索结果直接复用。满足写入门槛准备新增经验或复核条目时才读取 `references/recording.md`，安装、迁移或定位数据路径时读取 `references/maintenance.md`。周期扫描按需执行，不会自动创建定时任务。

检索与记录独立判断：修复 BUG 不等于自动入库。只记录非显然、已验证且可跨项目复用的经验；普通业务修正和未经验证的推测不写入。若全局或项目规则仍要求“每个项目任务开始检索”，需要同步收窄，否则仍会频繁触发。
...
复核扫描脚本只报告待复核、已经超过复核日期及生命周期字段异常的条目，不会自动修改或删除内容。字段异常可用校验脚本定位；围栏代码块未闭合时，检索和扫描会报错，不会误报为无匹配或无待处理项。建议每月运行一次：
